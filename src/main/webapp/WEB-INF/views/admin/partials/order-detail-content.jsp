...
<section class="admin-content__section admin-page admin-page--order-detail">
    <div class="page-header">
        ...
        <c:url var="adminBackToOrdersUrl" value="/admin/manage-orders">
            <c:if test="${not empty searchKeyword}">
                <c:param name="keyword" value="${searchKeyword}" />
            </c:if>
        </c:url>
        <a href="${adminBackToOrdersUrl}" class="btn btn-outline-secondary">
    ...
    <c:if test="${param.error == 'notAllowed'}">
        <c:set var="popupMessage" scope="request" value="This order is already handled by someone else, so you cannot update it." />
        <c:set var="popupType" scope="request" value="error" />
    </c:if>
        ...
        <div class="p-4">
            <form action="${pageContext.request.contextPath}/admin/manage-orders" method="post">
                ...
                <div class="d-grid gap-3">
                    <div>
                        <label class="form-label" for="adminPaymentStatus">Payment Status</label>
                        <select id="adminPaymentStatus" name="paymentStatus" class="action-select w-100" ${not canEditOrder ? 'disabled' : ''}>
                    ...
                    <div>
                        <label class="form-label" for="adminOrderStatus">Order Status</label>
                        <select id="adminOrderStatus" name="orderStatus" class="action-select w-100" ${not canEditOrder ? 'disabled' : ''}>
                ...
                <c:if test="${not canEditOrder}">
                    <p class="text-muted small mb-0 mt-3">This order can only be viewed here because it is already handled by another manager.</p>
                </c:if>
